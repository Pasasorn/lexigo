...
function getSheet(name) {
  ...
  if (!sh) {
    ...
    if (name === SHEET_STUDENTS) {
      sh.appendRow(['student_code','nickname','email','status','registered_at','added_by']);
...
function actionNewPurchase(p, cb) {
  ...
  if (!name || !phone || !pkg)
    return respond({status:'error', msg:'ข้อมูลไม่ครบ (name/phone/pkg)'}, cb);

  const orderId = 'ORD-' + Date.now();
  const sh = getSheet(SHEET_PURCHASES);
  sh.appendRow([orderId, name, phone, line, email, pkg, price, type, 'pending', '', new Date().toISOString(), '']);

  return respond({status:'ok', order_id: orderId}, cb);
}

// ── uploadSlip: รับสลิป base64 → บันทึกไฟล์ใน Google Drive ─
// POST body: { action:'uploadSlip', phone:'...', slip:'data:image/jpeg;base64,...' }
function actionUploadSlip(p, cb) {
  const phone = (p.phone || '').toString().trim();
  const slip  = (p.slip  || '').toString().trim();

  if (!phone || !slip) return respond({status:'error', msg:'ข้อมูลไม่ครบ'}, cb);
  ...
  try {
    ...
    // อัปเดต slip_url ใน Purchases (หา row ล่าสุดของเบอร์นี้)
    const sh   = getSheet(SHEET_PURCHASES);
    const data = sh.getDataRange().getValues();
    for (let i = data.length - 1; i >= 1; i--) {
      if (data[i][2].toString() === phone && data[i][8] === 'pending') {
...
function actionVerifyPurchase(p, cb) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const stSh = getSheet(SHEET_STUDENTS);
    stSh.appendRow([code, name, email, 'pending', new Date().toISOString(), 'purchase:' + orderId]);

    // อัปเดต Purchases row
    sh.getRange(i + 1, 9).setValue('verified');
    sh.getRange(i + 1, 12).setValue(code);

    return respond({status:'ok', student_code: code, name, email}, cb);
  }
  return respond({status:'not_found'}, cb);
}
