...
function getSheet(name) {
  ...
  if (!sh) {
    ...
    if (name === SHEET_CONFIG) {
      ...
      sh.appendRow(['site_url','https://peekaword.pages.dev/English/']);  // ← ใส่เบอร์/เลขบัญชีของครู
...
function doPost(e) {
  try {
    ...
    if (p.action === 'verifyPurchaseSlip') {
      const r = actionVerifyPurchaseSlip(p, cb);
      return r;
    }
...
// เขียนผลตรวจสลิปลง Purchases คอลัมน์ 13 เพื่อให้หน้าเว็บ poll อ่านได้
function setOrderNote_(orderId, state, msg, code) {
  try {
    const sh = getSheet(SHEET_PURCHASES);
    const d  = sh.getDataRange().getValues();
    for (let i = 1; i < d.length; i++) {
      if (d[i][0].toString() === orderId) {
        sh.getRange(i + 1, 13).setValue(state + '|' + (msg || ''));
        if (code) sh.getRange(i + 1, 12).setValue(code);
        if (state === 'reject') sh.getRange(i + 1, 9).setValue('rejected');
        return;
      }
    }
  } catch (e) {}
...
// ?action=orderStatus&order_id=ORD-...
function actionOrderStatus(p, cb) {
  const id = (p.order_id || '').toString().trim();
  if (!id) return respond({ status: 'error', msg: 'ระบุ order_id' }, cb);
  const sh = getSheet(SHEET_PURCHASES);
  const d  = sh.getDataRange().getValues();
  for (let i = d.length - 1; i >= 1; i--) {
    if (d[i][0].toString() === id) {
      const note = (d[i][12] || '').toString().split('|');
      return respond({
        status: 'ok',
        order_status: d[i][8],
        student_code: d[i][11] || '',
...
function verdict_(orderId, state, obj, cb) {
  setOrderNote_(orderId, state, obj.msg || '', '');
  return respond(obj, cb);
}
...
function actionVerifyPurchaseSlip(p, cb) {
  ...
  if (!KEY || KEY === 'YOUR_SLIPOK_KEY_HERE')
    return verdict_(orderId, 'manual', {status:'manual', msg: 'ยังไม่ได้ตั้งค่า SlipOK — ส่งให้ครูตรวจแทน' }, cb);
  ...
  try {
    ...
    return verdict_(orderId, 'manual', {status:'manual', msg: 'ตรวจอัตโนมัติไม่ได้ — ส่งให้ครูตรวจ' }, cb);
  }

  if (!result || !result.success || !result.data)
    return verdict_(orderId, 'reject', {status:'reject',
  ...
  // ── ชั้น 3: สลิปเก่าซ้ำ ─────────────────────────────────
  if (!transRef)
    return verdict_(orderId, 'manual', {status:'manual', msg: 'อ่านเลขอ้างอิงไม่ได้ — ส่งให้ครูตรวจ' }, cb);

  const slipSh = getSheet(SHEET_SLIPS);
  const slipData = slipSh.getDataRange().getValues();
  for (let i = 1; i < slipData.length; i++) {
    if (slipData[i][0].toString() === transRef)
      return verdict_(orderId, 'reject', {status:'reject', msg: 'สลิปนี้ถูกใช้ไปแล้ว กรุณาใช้สลิปการโอนครั้งใหม่' }, cb);
  }
  ...
  try {
    if (d.transDate) {
      ...
      if (!isNaN(td) && (Date.now() - td.getTime()) > 24 * 3600 * 1000)
        return verdict_(orderId, 'manual', {status:'manual', msg: 'สลิปเก่ากว่า 24 ชม. — ส่งให้ครูตรวจ' }, cb);
    }
